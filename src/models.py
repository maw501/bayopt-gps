...
def initialize_model(encoder_depth, n_c, freeze_base, pretrained=True):
    assert not (
        (pretrained == False) and (freeze_base is True)
    ), "Don't freeze base and start with random weights"
    model = ResnetBase(encoder_depth, pretrained=pretrained)
    set_parameter_requires_grad(
        model, freeze_base
    )  # train whole resnet base or not
...
class ResnetBase(nn.Module):
    def __init__(self, encoder_depth, pretrained=False):
        ...
        self.input_adjust = nn.Sequential(
            self.rn.conv1, self.rn.bn1, self.rn.relu
        )
